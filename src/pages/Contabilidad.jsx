// ============================================================
// pages/Contabilidad.jsx
// Pantalla de contabilidad: cobros, pendientes, pagos
// ============================================================
...
function Contabilidad() {
  ...
  const filtrarAño = (lista) => lista.filter(x => parseInt(x.año) === añoSelec);

  const totalGenerado  = filtrarAño(sesiones).reduce((acc, s) => acc + (parseFloat(s.precioTotal) || 0), 0);
  const totalCobrado   = filtrarAño(pagos).reduce((acc, p) => acc + (parseFloat(p.importe) || 0), 0);
  ...
  const pagosAño = filtrarAño(pagos).sort((a, b) => b.fecha.localeCompare(a.fecha));
    ...
    <div>
      ...
      <div className="stats-grid">
        <div className="stat-card">
          ...
          <div className="stat-label">Total generado {añoSelec}</div>
        </div>
      ...
      <div className="card">
        ...
        <div className="form-group">
          ...
          <input
            type="text"
            className="form-control"
            placeholder="Ej: Pago mayo 2026, Liquidación curso A..."
      ...
      {/* ── Historial de pagos ─────────────────────────────── */}
      <div className="card">
        <div className="card-title">🧾 Historial de pagos {añoSelec}</div>
        {pagosAño.length === 0 ? (
          <p style={{ textAlign: 'center', color: '#999', padding: '24px' }}>
            No hay pagos registrados en {añoSelec}
          </p>
        ) : (
