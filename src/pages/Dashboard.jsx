...
// Sincronizar Calendar: elimina automáticamente en Sheets
// las sesiones cuyos eventos ya no existen en Calendar
// ============================================================
...
function Dashboard() {
  ...
  const sincronizarCalendar = async () => {
    ...
    try {
      ...
      // ── 1. Eliminar automáticamente en Sheets las sesiones
      //       importadas desde Calendar que ya no existen allí
      // Ordenamos por _fila descendente para que eliminar filas
      // superiores no desplace el índice de las inferiores
      const sesionesAEliminar = sesionesActuales
        .filter(s => {
          if (!s.calendarEventId) return false;
          // Solo las importadas desde Calendar (id empieza por CAL)
          if (!s.id.startsWith("CAL")) return false;
          const ids = s.calendarEventId.split(",").map(id => id.trim()).filter(Boolean);
          // Si TODOS sus eventIds han desaparecido de Calendar → eliminar
          return ids.length > 0 && ids.every(id => !idsEnCalendar.has(id));
        })
        .sort((a, b) => b._fila - a._fila); // descendente
      ...
      // Recargamos sesiones actualizadas tras las eliminaciones
      const sesionesTrasBorrado  = await getSesiones();
      const mapaActualizado      = {};
      sesionesTrasBorrado.forEach(s => {
        if (s.calendarEventId) {
          s.calendarEventId.split(",").forEach(id => {
            mapaActualizado[id.trim()] = s;
          });
        }
      });

      // ── 2. Importar / actualizar eventos de Calendar → Sheets
      for (const evento of eventos) {
        ...
        const sesionExistente = mapaActualizado[evento.id];
      ...
      if (eliminados  > 0) partes.push(`${eliminados} sesión(es) eliminada(s) automáticamente`);
      if (ignorados   > 0) partes.push(`${ignorados} evento(s) futuro(s) omitido(s)`);
  ...
  const sesionesDelMes = sesiones.filter(s => parseInt(s.mes) === mesHoy && parseInt(s.año) === añoHoy);
