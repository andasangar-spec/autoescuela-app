...
function Dashboard() {
  ...
  const sincronizarCalendar = async () => {
    ...
    setProgresoSync("Conectando con Google Calendar...");
    ...
    try {
      ...
      setProgresoSync("Leyendo eventos del calendario...");
      ...
      const nuevasFilas = []; // acumular filas para inserción en lote

      setProgresoSync("Analizando eventos...");

      // ── Detectar sesiones eliminadas en Calendar ──────────
      sesionesActuales.forEach(s => {
        if (!s.calendarEventId || !s.id.startsWith("CAL")) return;
        const ids = s.calendarEventId.split(",").map(id => id.trim());
        if (ids.every(id => !idsEnCalendar.has(id))) {
      ...
      for (let i = 0; i < eventos.length; i++) {
        ...
        setProgresoSync(`Procesando evento ${i + 1} de ${eventos.length}...`);
        ...
        const precioTotal = tipoMatch.tipoPrecio === "total" ? tipoMatch.precio : numClases * tipoMatch.precio;
        ...
        if (sesionExistente) {
          ...
          if (!mismaHoraInicio || !mismaHoraFin || !mismaFecha || !mismoTipo) {
            ...
            await actualizarSesion({
              ...
              tipoCurso: titulo, horaInicio1: horaInicio, horaFin1: horaFin,
              horasTramo1: horasTotal.toFixed(2), horasTramo2: "0",
              horasTotal: horasTotal.toFixed(2),
              tipoPrecio: tipoMatch.tipoPrecio === "total" ? "total" : "clase",
              precioHora: tipoMatch.precio, precioTotal: precioTotal.toFixed(2),
              notas: sesionExistente.notas || "Importado desde Google Calendar",
            });
            actualizados++;
          }
        } else {
          // Acumular para inserción en lote
          nuevasFilas.push([
            generarId("CAL"), fechaFmt, diaSemana, semana, mes, año,
            titulo, horaInicio, horaFin, "NO", "", "",
            horasTotal.toFixed(2), "0", horasTotal.toFixed(2),
            tipoMatch.tipoPrecio === "total" ? "total" : "clase",
            tipoMatch.precio, precioTotal.toFixed(2),
            evento.id, "Importado desde Google Calendar",
          ]);
          importados++;
        }
      }

      // ── Insertar nuevas sesiones en lote ──────────────────
      if (nuevasFilas.length > 0) {
        setProgresoSync(`Guardando ${nuevasFilas.length} sesión(es) en Sheets...`);
        // Insertar en bloques de 10 para no saturar la API
        const BLOQUE = 10;
        for (let i = 0; i < nuevasFilas.length; i += BLOQUE) {
          const bloque = nuevasFilas.slice(i, i + BLOQUE);
          await guardarSesionesLote(bloque);
          if (i + BLOQUE < nuevasFilas.length) {
            await new Promise(r => setTimeout(r, 500)); // pausa entre bloques
          }
        }
      }
      ...
      if (eliminadasDetectadas.length > 0) partes.push(`${eliminadasDetectadas.length} eliminada(s) en Calendar — revisa los avisos`);
  ...
  return (
    <div>
      <div className="page-header">
        ...
        <div style={{ display:"flex", gap:"8px", flexWrap:"wrap" }}>
          ...
          <button
            ...
            {sincronizando
              ? <><div className="spinner" style={{ width:"16px",height:"16px",borderWidth:"2px" }}></div> {progresoSync || "Sincronizando..."}</>
      ...
      {/* Avisos eliminados en Calendar */}
      {sesionesEliminadas.length > 0 && (
        <div className="card" style={{ border:"2px solid #ea4335" }}>
          <div className="card-title" style={{ color:"#ea4335" }}>⚠️ Eventos eliminados en Google Calendar</div>
          <p style={{ fontSize:"13px", color:"#666", marginBottom:"16px" }}>
            Los siguientes eventos fueron eliminados en Calendar. ¿Quieres eliminar también las sesiones en la app?
          </p>
          {sesionesEliminadas.map((s, i) => (
            <div key={i} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"12px", background:"#fce8e6", borderRadius:"8px", marginBottom:"8px", gap:"12px" }}>
              ...
              <div style={{ display:"flex", gap:"8px", flexShrink:0 }}>
                <button className="btn btn-outline" style={{ padding:"6px 12px", fontSize:"12px" }}
                  onClick={() => ignorarAviso(s.id)}>Ignorar</button>
                <button className="btn btn-danger" style={{ padding:"6px 12px", fontSize:"12px", justifyContent:"center" }}
      ...
      <div className="card">
        ...
        {ultimasSesiones.length === 0 ? (
          <p style={{ color:"#999", textAlign:"center", padding:"24px" }}>No hay sesiones registradas aún</p>
        ) : (
...
// ── Insertar múltiples filas en lote ─────────────────────────
async function guardarSesionesLote(filas) {
  const SPREADSHEET_ID = "1LHCwfVH39txuID55bSk9mRrpErdGAa9v0sRqTgI0_2A";
  const token = localStorage.getItem("google_token");
  const url = `https://sheets.googleapis.com/v4/spreadsheets/${SPREADSHEET_ID}/values/SESIONES!A1:T1000:append?valueInputOption=RAW&insertDataOption=INSERT_ROWS`;
  const res = await fetch(url, {
    method: "POST",
    headers: { "Authorization": `Bearer ${token}`, "Content-Type": "application/json" },
    body: JSON.stringify({ values: filas }),
  });
  if (!res.ok) throw new Error("Error añadiendo filas en SESIONES");
  return res.json();
}
