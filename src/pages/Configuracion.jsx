...
// Gestión de tipos de curso, precios y configuración
// ============================================================
...
function Configuracion() {
  ...
  const [editando, setEditando] = useState(null); // índice del tipo en edición
  ...
  const cargarTipos = async () => {
    ...
    try {
      ...
      setError('Error al cargar tipos de curso');
    } finally {
      setCargando(false);
    }
  };
    ...
    <div>
      ...
      <div className="card">
        ...
        <p style={{ fontSize: '13px', color: '#666', marginBottom: '16px' }}>
          Modifica los precios y configuración de cada tipo de clase. Los cambios se guardan en tu Google Sheet.
        </p>
        ...
        <div style={{ overflowX: 'auto' }}>
          <table>
            ...
            <tbody>
              {tipos.map((tipo, idx) => (
                <tr key={idx} style={{ background: editando === idx ? '#fffde7' : '' }}>
                  ...
                  <td>
                    <input
                      type="number"
                      value={tipo.precio}
                      onChange={e => handleCambio(idx, 'precio', parseFloat(e.target.value))}
                      min="0"
                      step="0.5"
                      style={{
                        border: '1px solid #e0e0e0',
